...
python dedup_logo_from_manifest.py \
  ...
  --outdir /Users/ecreed/Desktop/KelloggRotation/CodeProjects/TagmentationAnalysis/02022026_TagmentationAnalysis/Test_analysis_pipeline/test_output/test_dedup \
  --flank 100 \
  --logo-type bits \
  --logo-name 02022026_dedup_recapLogoFromLiz_test_W1DN1_analysis_logo
  '''
...
def main():
    ...
    ap.add_argument(
        "--weighting",
        choices=["reads", "unique_site", "unique_site_strand"],
        default="unique_site_strand",
        help=(
            "How to weight insertion windows for the logo. "
            "'reads' = one window per TSV row (coverage-weighted). "
            "'unique_site' = one window per (ref_fasta, contig, ins0) (coverage-independent). "
            "'unique_site_strand' = one window per (ref_fasta, contig, ins0, strand) (coverage-independent)."
        ),
    )
    ...
    for _, row in manifest.iterrows():
        ...
        for tsv in glob.glob(tsv_path):
            ...
            for _, r in df.iterrows():
                ...
                # Coverage-independent motif option: keep one window per unique site
                if args.weighting == "reads":
                    key = None
                elif args.weighting == "unique_site":
                    key = (ref_fasta, contig, ins0)
                else:  # unique_site_strand
                    key = (ref_fasta, contig, ins0, strand)
