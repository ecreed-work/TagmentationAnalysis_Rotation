...
python dedup_reads_logo_from_manifest.py \
  ...
  --flank 100 \
  --logo-type bits \
  --logo-name dedup_W1DN1_analysis_logo+reads
  '''
...
def main():
    ...
    for _, row in manifest.iterrows():
        ...
        for tsv in glob.glob(tsv_path):
            ...
            for _, r in df.iterrows():
                ...
                site_key = (contig, ins0)
                if site_key in unique_sites:
                    continue
                unique_sites.add(site_key)
                windows_used += 1
    ...
    logo.ax.set_title(
        ...
        f"Reads processed: {reads_processed} | Unique sites: {windows_used} | Windows used: {len(windows)}"
    )
    plt.tight_layout()
    ...
    print(f"[ok] windows: {len(windows)} (unique insertion sites: {windows_used})")
