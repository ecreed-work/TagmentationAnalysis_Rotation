...
python targetstrand_logo_from_manifest.py \
  --manifest-xlsx /Users/ecreed/Desktop/KelloggRotation/CodeProjects/TagmentationAnalysis/02022026_TagmentationAnalysis/Test_analysis_pipeline/test_output/test_recap/filtered_tsv_manifest.xlsx \
  --outdir /Users/ecreed/Desktop/KelloggRotation/CodeProjects/TagmentationAnalysis/02022026_TagmentationAnalysis/Test_analysis_pipeline/test_output/test_recap \
  --sgrna GCTGGCGCATGATGACCACCGATA \
  --flank 100 \
  --logo-type bits \
  --logo-name 02022026_TargetStrandRecapLogoFromLiz_test_W1DN1_analysis_logo
  '''
...
def find_sgrna_binding_strand(ref_seq: str, sgrna: str) -> str:
    """
    Return '+' if sgRNA matches forward strand, '-' if revcomp(sgRNA) matches forward.
    Target strand is the opposite.
    """
    guide = sgrna.upper().replace("U", "T")
    seq_u = ref_seq.upper()
    ...
    if rc in seq_u:
        return "-"
    raise ValueError("sgRNA sequence not found in reference genome")
...
def main():
    ...
    ap.add_argument("--sgrna", required=True, help="sgRNA protospacer sequence (DNA, no PAM)")
    ...
    for _, row in manifest.iterrows():
        ...
        if ref_fasta not in target_strand_cache:
            first_contig = next(iter(ref_dict.keys()))
            binding_strand = find_sgrna_binding_strand(ref_dict[first_contig], args.sgrna)
            target_strand_cache[ref_fasta] = "-" if binding_strand == "+" else "+"
        target_strand = target_strand_cache[ref_fasta]
        ...
        for tsv in glob.glob(tsv_path):
            ...
            for _, r in df.iterrows():
                ...
                # Use target strand sequence for all windows
                if target_strand == "-":
                    w = revcomp(w)
    ...
    logo.ax.set_title(
        f"{args.logo_name} — target strand sequence logo ({args.logo_type})"
    )
    plt.tight_layout()
